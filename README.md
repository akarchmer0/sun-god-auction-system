...
Sun God Auction Systems is a local-first fantasy football auction draft room with a voice auctioneer, zero-install phone bidding, salary-cap enforcement, live rosters, and a reversible sale ledger.
...
- `src/app.mjs` coordinates the host room, phone/voice bid decisions, and the auction UI.
...
- `server.mjs` serves the host and phone pages, broadcasts local room events, mints short-lived OpenAI Realtime session tokens, and interprets bid transcripts; the permanent API key remains server-side.
