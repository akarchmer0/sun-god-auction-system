...
Continuous play-by-play is on by default. With `OPENAI_API_KEY` configured, the AI Patter Director prepares exactly one short, funny phrase at a time using the player, current leader, price, next bid, suggested value, budget, roster, bid count, and recent sales. Its prompt favors playful fantasy-football punchlines with the energy of elite Latin American soccer commentary, without imitating an accent or inventing football facts. Every price, phase, or leader change invalidates the phrase before it can become stale. If OpenAI is unavailable or slow, Lucy immediately uses one funny phrase from the local rotating script instead. Set `OPENAI_PATTER_MODEL` to override the default `gpt-5.6-luna` director.
...
- `src/patter-director.mjs` bounds the live context and owns the single-phrase momentum prompt and strict output contract.
...
- `src/auctioneer-voice.mjs` plays streamed PCM with pitch-preserving time compression, giving ElevenLabs an additional 1.08–1.30× pace boost and Cartesia a lighter 1.00–1.10× boost without raising the voice pitch.
